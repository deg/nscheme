//! Print every `(test ...)` failure recorded by the chibi conformance
//! suite, in source order. Useful for triaging regressions:
//! `cargo run --example dump_fails`.
...
fn main() {
    std::thread::Builder::new()
        ...
        .spawn(|| {
            ...
            // The shim accumulates failures into `$failures` as a
            // cons-list, newest-first. Reverse to show source order.
