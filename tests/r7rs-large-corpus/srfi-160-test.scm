;;; SRFI 160 reference test suite (shared-tests.scm, John Cowan/Shiro Kawai,
;;; MIT), the s16 instantiation. Preamble adapted: (chibi test)->(srfi 64),
;;; sub1 from the chibi driver inlined. Assertions verbatim.
(import (scheme base) (scheme write) (srfi 64) (srfi 128) (srfi 160 s16))
(define (sub1 x) (- x 1))
(test-begin "srfi-160-s16")
...
(test-group "s16vector/iteration"
  ...
  ;; nscheme: s16vectors are record-backed, so `equal?` (which `test`
  ;; uses) does not compare them by contents the way it does the native
  ;; @vectors this assertion assumes. Compare element lists instead;
  ;; the segment values themselves are verbatim.
  (test "segment" (list '(1 2 3) '(4 5))
