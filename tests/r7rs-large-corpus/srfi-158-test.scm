;;; SRFI 158 reference test suite (Shiro Kawai/John Cowan/Thomas Gilray).
;;; Adapted: chibi/chicken preamble -> (srfi 64); srfi-4 (unused) and the
;;; r7rs-shim (all R7RS base) dropped. Assertions verbatim.
(import (scheme base) (srfi 64) (scheme generator) (only (srfi 1) unfold))
(test-begin "srfi-158")
...
(test-group "generators"
  ...
  (test-group "generators/consumers"
    ...
    (test '(e d c b a . z) (let ((p (open-input-string "a b c d e")))
                             (generator-fold cons 'z (lambda () (read p)))))
