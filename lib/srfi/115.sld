;;; (srfi 115) — Regular Expressions (SRFI 115, R7RS-large Red Edition).
;;;
;;; Vendored from the SRFI 115 reference implementation by Alex Shinn,
;;; BSD-3-Clause (text preserved below). The upstream .sld uses
;;; (include "115.scm") and (import (srfi 115 boundary)), where the
;;; boundary sub-library itself does (include "boundary.scm"). nscheme
;;; resolves `include` relative to the process directory rather than the
;;; library file, so both body files are inlined here inside `begin`
;;; instead of being included.
;;;
;;; SPDX-FileCopyrightText: 2013 - 2016 Alex Shinn
;;; SPDX-License-Identifier: BSD-3-Clause
;;;
;;; boundary.scm: SPDX-FileCopyrightText: 2015 Alex Shinn / CC0-1.0
;;;
;;; ───────────────────────────────────────────────────────────────────
;;; STATUS FOR nscheme: BLOCKED — does not load in current nscheme.
;;;
;;; This library is fundamentally built on SRFI 14 (character sets): the
;;; reference implementation has ~141 char-set operations and the entire
;;; NFA engine dispatches on char-sets. nscheme has NO SRFI 14, so the
;;; `(import (scheme base) (scheme char) (srfi 1) (srfi 14))` clause in
;;; the `else` branch below cannot be resolved. It additionally needs
;;; bitwise ops from SRFI 60 (bitwise-and / bitwise-ior / bitwise-not),
;;; also absent. SRFI 1 IS vendored (lib/srfi/1.sld) so it is fine.
;;;
;;; The cond-expand structure is preserved verbatim from upstream so that
;;; once SRFI 14 and a bitwise SRFI (60/33/151) are vendored, this file
;;; should load as-is. See the integrator notes / missing_features.
;;; ───────────────────────────────────────────────────────────────────
  ...
  (cond-expand
   ...
   (else
    (begin
      ...
      (define (string-concatenate-reverse ls)
        (string-concatenate (reverse ls))))))
  ;; (srfi 115 boundary) inlined below (upstream: (import (srfi 115 boundary))).
  ;; NOTE: these char-set:* definitions ALSO require SRFI 14.
  (begin
;; Control
