...
;;;
;;; Vendored from the SRFI 1 reference implementation by Olin Shivers
;;; (1998, 1999), MIT licence (text preserved below). Flattened into a
;;; single file: the reference is a bare `srfi-1-reference.scm` body that
;;; assumes a host supplying `receive`, `let-optionals`, `:optional` and
;;; `check-arg`. nscheme resolves `include` relative to the process
;;; directory, so rather than rely on includes we inline the whole body
;;; here inside `begin`, prefixed by a small portability prelude that
;;; defines those four host hooks in terms of (scheme base).
;;;
...
(define-library (srfi 1)
  (export
   ...
   ;; Primitive side-effects (re-exported from base)
      ...
      (syntax-rules ()
        ((receive formals expression body ...)
         (call-with-values (lambda () expression)
           (lambda formals body ...)))))
    ;; :optional rest default — pick the first optional arg, or default.
    (define-syntax :optional
      (syntax-rules ()
        ((:optional rest default)
         (if (pair? rest) (car rest) default))))
    ;; let-optionals rest ((var default) ...) body — bind positional optionals.
      ...
      (syntax-rules ()
        ...
        ((%let-optionals rest ((var default) more ...) body ...)
         (let ((var (if (pair? rest) (car rest) default))
               (next (if (pair? rest) (cdr rest) '())))
           (let ((rest next))
             (%let-optionals rest (more ...) body ...))))))
    ;; check-arg: loop until pred holds, signalling on failure (per the
    ;; reference's documented definition).
    (define (check-arg pred val caller)
      (if (pred val) val (error "Bad argument" val pred caller)))
    ;; ---- begin SRFI 1 reference implementation body ---------------------
