;;; (srfi 117) — Mutable queues backed by lists.
;;;
;;; Vendored from the SRFI 117 reference implementation. The .sld and
;;; copyright are by John Cowan (2014, MIT); the implementation body
;;; (list-queues-impl.scm) is by Alex Shinn (2017, BSD-3-Clause), taken
;;; in part from Chibi Scheme. Both license texts are preserved below.
;;;
;;; Flattened into a single file: the upstream .sld uses
;;; (include "list-queues-impl.scm"), but nscheme resolves `include`
;;; relative to the process directory, so the body is inlined here
;;; inside `begin` instead.
;;;
;;; The upstream body opens with three "R7RS shims" (make-list,
;;; list-copy, list-set!) and notes "Comment these out on an R7RS
;;; system." nscheme is an R7RS system — all three live in
;;; (scheme base) — so they are omitted here, per that instruction.
;;; (Only list-copy is actually used by the body; make-list and
;;; list-set! are dead code upstream.) The SRFI-1 helpers last-pair
;;; and map! are NOT in (scheme base), so they are retained.
;;;
