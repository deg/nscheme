...
;;;
;;; Vendored from the SRFI 14 reference implementation: ported from the
;;; MIT Scheme runtime by Brian D. Carlstrom, rehacked and extended by
;;; Olin Shivers (1998, 2000). Flattened into a single file: the upstream
;;; source (srfi-14.scm) is loaded by an external module form, so its body
;;; is inlined here inside `begin` rather than via `include` (nscheme
;;; resolves `include` relative to the process directory, not this file).
;;;
;;; Char sets are represented as 256-char strings over Latin-1: index I
;;; holds char code 1 if I is in the set, 0 otherwise. This makes the
;;; library Latin-1 only — see the missing_features notes.
;;;
;;; The non-R5RS dependencies named in the upstream header (ERROR,
;;; %LATIN1->CHAR / %CHAR->LATIN1, LET-OPTIONALS* / :OPTIONAL, BITWISE-AND,
;;; DEFINE-RECORD-TYPE, CHECK-ARG) are supplied as small shims in the first
;;; `begin` block below, since nscheme provides only some of them.
;;;
;;; SPDX-License-Identifier: MIT-derived (MIT Scheme license, text below).
;;;
...
(define-library (srfi 14)
  ...
  ;; ---- Shims for the upstream non-R5RS dependencies -------------------
  ;; nscheme lacks bitwise-and, let-optionals*/:optional, %latin1->char,
  ;; %char->latin1 and check-arg as built-ins; supply minimal versions.
  ;; (error, define-record-type, values/call-with-values are built in.)
  (begin

    ;; Latin-1 codec: char sets only ever index/store codes 0..255, so
    ;; integer->char / char->integer are exact substitutes.
    (define (%latin1->char n) (integer->char n))
    (define (%char->latin1 c) (char->integer c))

    ;; Argument check: returns VAL if (PRED VAL), else raises.
    (define (check-arg pred val caller)
      (if (pred val) val (error "Bad argument" val caller)))

    ;; bitwise-and for non-negative integers. char-set-hash only calls it
    ;; as (bitwise-and MASK x) where MASK is 2^n-1, but this general
    ;; definition is correct for any two non-negative integers.
    (define (bitwise-and a b)
      (let loop ((a a) (b b) (bit 1) (ans 0))
        (if (or (zero? a) (zero? b))
            ans
            (loop (quotient a 2)
                  (quotient b 2)
                  (* bit 2)
                  (if (and (odd? a) (odd? b)) (+ ans bit) ans)))))

    ;; (%optional REST DEFAULT [PRED]) — single-optional-argument helper,
    ;; standing in for the upstream :OPTIONAL macro.
    (define-syntax %optional
      (syntax-rules ()
        ((%optional rest default)
         (if (pair? rest) (car rest) default))
        ((%optional rest default pred)
         (if (pair? rest)
             (check-arg pred (car rest) '%optional)
             default))))

    ;; (let-optionals* REST ((var default) ... tail) body ...) — minimal
    ;; version covering the single upstream use shape:
    ;;   (let-optionals* rest ((error? #f) rest) body ...)
    (define-syntax let-optionals*
      (syntax-rules ()
        ((let-optionals* rest () body ...)
         (begin body ...))
        ((let-optionals* rest (tail) body ...)
         (let ((tail rest)) body ...))
        ((let-optionals* rest ((var default) rest2 ...) body ...)
         (let ((var (if (pair? rest) (car rest) default))
               (%rest-rest (if (pair? rest) (cdr rest) '())))
           (let-optionals* %rest-rest (rest2 ...) body ...))))))

  ;; ---- Flattened reference implementation -----------------------------
  (begin

    ;; Upstream names this record :char-set; renamed to char-set-rec so
    ;; the tag is a plain identifier (no colon prefix).
    (define-record-type char-set-rec
      (make-char-set s)
      char-set?
      (s char-set:s))
    ...
    ;;; If CS is really a char-set, do CHAR-SET:S, otw report an error.
    ...
    ;;; Internal accessors hiding the string representation.
    ...
    ;;; "s[i] := s[i] op val" operations -- see %CHAR-SET-ALGEBRA.
    ...
    ;;; in to keep the intermediate values small.
    ...
    (define (char-set-hash cs . maybe-bound)
      (let* ((bound (%optional maybe-bound 4194304 (lambda (n) (and (integer? n)
             ...
             (mask (let lp ((i #x10000)) ; Skip first 16 iterations.
    ...
    ;;; Cursors
    ;;; A cursor is an integer index into the mark vector, -1 = end.
    ...
    ;;; S := S OP CSETi for all the char-sets in the list CSETS.
